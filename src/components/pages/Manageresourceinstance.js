...
const Manageresourceinstance = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_BASE_URL}resources/create`,
        {
          method: 'POST',
          headers: {
  ...
  const handleDelete = async (resourceId) => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_BASE_URL}resources/${resourceId}`,
        {
          method: 'DELETE'
        }
      );
  ...
  const fetchCloudProviders = async () => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_BASE_URL}cloudprovider/list`
      );
      const data = await response.json();
  ...
  const [setSelectedCloudProvider] = useState('');
  const handleCloudChange = (event) => {
    const selectedCloud = event.target.value;
...
export default Manageresourceinstance;
